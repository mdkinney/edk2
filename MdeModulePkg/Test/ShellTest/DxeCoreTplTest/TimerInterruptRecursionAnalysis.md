...
occurs inside a timer interrupt handler (via `CoreTimerTick`'s lock release),
a pending timer interrupt immediately fires, recursively re-entering the
handler. This creates unbounded stack growth leading to stack overflow. The
root cause is that `CoreRestoreTpl()` cannot distinguish normal context from
interrupt context — it has no mechanism to know that interrupt re-enable
should be deferred to the interrupt-return instruction.

**Root Cause:** All CPU architectures disable interrupts on interrupt entry
(hardware guarantee). The DXE Core's `CoreRestoreTpl()` re-enables them in
software before the interrupt handler returns, creating a recursion window.
The CPU Architecture Protocol's `GetInterruptState()` — the natural way to
detect interrupt context — returns stale cached values on IA-32/X64 and
RISC-V, making ISR detection unreliable.

**Recommended Solution (Option F — Solution 4):** Two-part fix:
1. Fix `GetInterruptState()` in the IA-32/X64 and RISC-V CPU DXE drivers to
   read the actual hardware interrupt flag (spec conformance fix).
2. In `CoreRaiseTpl()`, call `gCpu->GetInterruptState()` to detect ISR
   context. Record the interrupted TPL in a bitmask. In `CoreRestoreTpl()`,
   use the mask to suppress interrupt re-enable while in ISR context — the
   interrupt-return instruction will restore them.

This bounds nesting to at most 1 additional level, works on all
architectures, requires no timer driver changes, eliminates false positives,
...
**Pattern A — Timer driver wraps with RaiseTpl/RestoreTpl (ArmPkg):**
...
**Pattern B — Timer driver does no TPL management (PcAtChipsetPkg):**
...
**Pattern C — Deferred RestoreTpl via NestedInterruptTplLib (OvmfPkg):**
...
### 2.1 Normal Operation (No Recursion)
...
sequenceDiagram
    ...
    Note over Tick: ⚠️ Interrupts now ENABLED<br/>inside CoreTimerTick!
...
### 2.2 Infinite Recursion (The Bug)
...
sequenceDiagram
    ...
    Note over T1: ⚠️ Still inside CoreTimerTick [1]<br/>but interrupts are ENABLED
    ...
    Note over T2: ⚠️ Still inside CoreTimerTick [2]<br/>interrupts ENABLED again
...
### 2.3 TPL Nesting and Interrupt Windows
...
### 2.4 Additional Recursion Path — Event Dispatch Loop
...
flowchart TD
    ...
    G -->|Yes| H["CoreSetInterruptState(TRUE)<br/>⚠️ Interrupts ENABLED"]
    ...
    L -->|Yes| M["CoreSetInterruptState(TRUE)<br/>⚠️ Interrupts ENABLED"]
...
conformance), then uses a `mIsrEntryTplMask` bitmask approach (extending
Solution 1 with bounded preemption) in the DXE Core. This eliminates the
false-positive scenarios (10, 15, 18, 20, 25) that affect Solutions 1–3
because `gCpu->GetInterruptState()` now returns the TRUE hardware state
...
**Part 2: DXE Core `mIsrEntryTplMask` (extends Solution 1 with bounded preemption)**

With `gCpu->GetInterruptState()` now reliable, the DXE Core uses an
interrupted TPL mask approach. `CoreRaiseTpl` calls
`gCpu->GetInterruptState()` to detect ISR context, and `CoreRestoreTpl`
uses the mask to guard interrupt re-enable in both the event dispatch loop
and the final section.  Unlike Solution 1's `== 0` check, the event loop
uses `HighBitSet64(mIsrEntryTplMask) < gEfiCurrentTpl` to allow bounded
preemption at levels above the highest interrupted TPL.

```c
// CoreRaiseTpl — detect ISR context via CPU Arch Protocol
...
// CoreRestoreTpl — event dispatch loop (bounded preemption)
...
// CoreRestoreTpl — final section (early-return pattern)
CoreSetInterruptState (FALSE);
gEfiCurrentTpl = NewTpl;
if (NewTpl >= TPL_HIGH_LEVEL) {
    return;
...
if ((INTN)NewTpl <= HighBitSet64 (mIsrEntryTplMask)) {
    mIsrEntryTplMask &= (1ULL << NewTpl) - 1;  // ISR: clear bits, stay disabled
    return;
}
...
Solutions 1–3 suffer from false positives in Scenarios 10, 15, 18, 20, 25
because `gCpu->GetInterruptState()` returned stale cached values. When code
called `gCpu->DisableInterrupt()` before `RaiseTpl(HIGH)`, the cached
variable was updated to FALSE, making the protocol report "disabled" —
indistinguishable from hardware-disabled-on-ISR-entry.

With the fix, `GetInterruptState()` reads actual hardware:

| Scenario | Hardware State | Protocol Returns | Mask Action |
|----------|--------------|-----------------|-------------|
| Normal `RaiseTpl(HIGH)` | IF=1 (enabled) | TRUE | No mask bit set |
| `DisableInterrupt()` → `RaiseTpl(HIGH)` | IF=0 (SW disabled) | FALSE | Mask bit set ⚠️ |
| ISR entry → `RaiseTpl(HIGH)` | IF=0 (HW disabled) | FALSE | Mask bit set ✅ |

The `DisableInterrupt()` → `RaiseTpl(HIGH)` case (Scenario 10) still reads
IF=0 from hardware, so it still sets a mask bit. However, this is actually
**correct behavior**: if code has explicitly disabled interrupts and then
raises TPL to HIGH, `RestoreTpl` should NOT re-enable interrupts because the
caller's explicit disable should be respected. The caller is responsible for
calling `EnableInterrupt()` when ready. This is spec-conformant: the
protocol's `DisableInterrupt()` is defined to actually disable hardware
interrupts, so `GetInterruptState()` correctly returns FALSE.

The key insight is that Solutions 1–3's "false positive" in Scenario 10 was
only a false positive because the old cached implementation returned FALSE
without actually reading hardware — it was *accidentally* correct for ISR
detection but *wrong* for the `DisableInterrupt()` case because the cache
could be stale in other scenarios. With the fix, the return value is always
the true hardware state, making the behavior **consistently correct**:

- Interrupts truly disabled (by HW or SW) → don't re-enable → correct
- Interrupts truly enabled → re-enable after `RestoreTpl` → correct

#### 3.6.4 False-Positive Resolution Table

| # | Scenario | Solutions 1–3 (cached) | Solution 4 (HW read) | Why Different |
|---|---|---|---|---|
| 10 | `DisableInterrupt()` → `RaiseTpl(HIGH)` | ⚠️ False positive | ✅ Correct | HW reads IF=0 → respects caller's explicit disable |
| 15 | App `DisableInterrupt()` → `RaiseTpl(HIGH)` | ⚠️ False positive | ✅ Correct | Same — caller disabled, RestoreTpl respects it |
| 18 | App disables IRQ, later `RaiseTpl(HIGH)` | ⚠️ False positive | ✅ Correct | Same — IF=0 from explicit disable |
| 20 | `CLI` → `RaiseTpl(HIGH)` | ⚠️ False positive | ✅ Correct | CLI sets IF=0 → HW reads FALSE → correct |
| 25 | CLI/STI bracket with `RaiseTpl` inside | ⚠️ Partial false pos. | ✅ Correct | Inside CLI bracket, IF=0 → correct |

**Note:** For Scenarios 20 and 25 (raw `CLI` without protocol call), the
behavior depends on whether the CPU Arch Protocol's cache is also stale.
With the fix reading hardware directly, `CLI` correctly results in FALSE
regardless of the cached variable state.
...
#### 3.6.6 Bounded ISR Nesting — Sequence Diagram

Solution 4's bounded preemption allows ISR nesting up to the number of
dispatch TPL levels (3 with standard UEFI: APPLICATION, CALLBACK, NOTIFY).
The `mIsrEntryTplMask` bitmask tracks which levels have been interrupted and
the `HighBitSet64(mIsrEntryTplMask) < gEfiCurrentTpl` condition in the event
dispatch loop re-enables interrupts only when dispatching strictly above the
highest interrupted level.  This ensures each nested ISR interrupts at a
strictly higher TPL, consuming one mask bit per level.  Once all dispatch
levels have been consumed, no further interrupt re-enable is possible,
guaranteeing termination.

The following diagram shows the **maximum nesting case**: normal code
interrupted at `TPL_APPLICATION` (4), with ISRs nesting through all
three dispatch levels.  The maximum simultaneous ISR stack depth is 3.

```mermaid
sequenceDiagram
    participant N as Normal Code<br/>(TPL_APP=4)
    participant ISR1 as ISR₁<br/>(interrupts APP)
    participant ISR2 as ISR₂<br/>(interrupts NOTIFY)
    participant ISR3 as ISR₃<br/>(interrupts CB)
    participant ISR4 as ISR₄<br/>(interrupts NOTIFY)

    Note over N: Running at TPL_APP (4)<br/>Interrupts enabled

    N->>ISR1: ⚡ TIMER IRQ (HW disables interrupts)
    activate ISR1
    Note over ISR1: RaiseTpl(HIGH): OldTpl=4<br/>GetInterruptState()→FALSE<br/>ASSERT: 4 >= HighBitSet64(0)=-1 ✓<br/>mask |= (1<<4) → mask={APP}<br/>CoreSetInterruptState(FALSE)

    Note over ISR1: RestoreTpl(APP) — event loop

    rect rgb(220, 240, 255)
        Note over ISR1: Dispatch NOTIFY (16):<br/>HighBitSet64({APP})=4 < 16<br/>→ CoreSetInterruptState(TRUE)

        ISR1->>ISR2: ⚡ TIMER IRQ during NOTIFY dispatch
        activate ISR2
        Note over ISR2: RaiseTpl(HIGH): OldTpl=16<br/>GetInterruptState()→FALSE<br/>ASSERT: 16 >= 4 ✓<br/>mask |= (1<<16) → mask={APP,NOTIFY}
        Note over ISR2: RestoreTpl(NOTIFY):<br/>HighBitSet64({APP,NOTIFY})=16<br/>16 not < 16 → IRQs OFF<br/>Final: 16≤16 → mask &= (1<<16)-1<br/>mask → {APP}. Return for IRET
        ISR2-->>ISR1: IRET (restores interrupts)
        deactivate ISR2
    end

    rect rgb(220, 255, 220)
        Note over ISR1: Dispatch CALLBACK (8):<br/>HighBitSet64({APP})=4 < 8<br/>→ CoreSetInterruptState(TRUE)

        ISR1->>ISR3: ⚡ TIMER IRQ during CALLBACK dispatch
        activate ISR3
        Note over ISR3: RaiseTpl(HIGH): OldTpl=8<br/>GetInterruptState()→FALSE<br/>ASSERT: 8 >= 4 ✓<br/>mask |= (1<<8) → mask={APP,CB}

        Note over ISR3: RestoreTpl(CB) — event loop

        rect rgb(255, 240, 220)
            Note over ISR3: Dispatch NOTIFY (16):<br/>HighBitSet64({APP,CB})=8 < 16<br/>→ CoreSetInterruptState(TRUE)

            ISR3->>ISR4: ⚡ TIMER IRQ during NOTIFY dispatch
            activate ISR4
            Note over ISR4: RaiseTpl(HIGH): OldTpl=16<br/>GetInterruptState()→FALSE<br/>ASSERT: 16 >= 8 ✓<br/>mask |= (1<<16)<br/>mask → {APP, CB, NOTIFY}
            Note over ISR4: RestoreTpl(NOTIFY):<br/>HighBitSet64({APP,CB,NOTIFY})=16<br/>16 not < 16 → IRQs OFF<br/>⛔ ALL LEVELS CONSUMED<br/>NO FURTHER NESTING POSSIBLE<br/>Final: 16≤16 → mask &= (1<<16)-1<br/>mask → {APP,CB}. Return for IRET
            ISR4-->>ISR3: IRET (restores interrupts)
            deactivate ISR4
        end

        Note over ISR3: Dispatch CALLBACK (8):<br/>HighBitSet64({APP,CB})=8<br/>8 not < 8 → IRQs OFF<br/>PendingTpl(8)≤NewTpl(8) → break<br/>Final: 8≤8 → mask &= (1<<8)-1<br/>mask → {APP}. Return for IRET
        ISR3-->>ISR1: IRET (restores interrupts)
        deactivate ISR3
    end

    Note over ISR1: Final: NewTpl=4, HighBitSet64({APP})=4<br/>4≤4 → mask &= (1<<4)-1<br/>mask → 0 (empty). Return for IRET
    ISR1-->>N: IRET (restores interrupts)
    deactivate ISR1

    Note over N: Normal execution continues<br/>mask=0, interrupts restored by IRET
```

**Maximum ISR stack depth at the deepest point (ISR₄ active):**

| Stack Frame | Interrupted at | mask bits set | Dispatch levels with IRQs ON |
|---|---|---|---|
| ISR₁ | APP (4) | {4} | NOTIFY (16), CALLBACK (8) |
| ISR₃ | CALLBACK (8) | {4, 8} | NOTIFY (16) |
| ISR₄ | NOTIFY (16) | {4, 8, 16} | **None** — all levels consumed |

**Why recursion terminates:** Each nested ISR sets a bit at a strictly higher
TPL (enforced by the ASSERT).  The event loop condition
`HighBitSet64(mask) < gEfiCurrentTpl` only enables interrupts at dispatch
levels above the highest set bit.  Once all three dispatch levels
(APPLICATION, CALLBACK, NOTIFY) have bits in the mask, every dispatch level
fails the `<` check and interrupts remain disabled — no further timer
interrupt can fire.  The maximum nesting depth equals the number of dispatch
TPL levels between the originally interrupted TPL and `TPL_HIGH_LEVEL`
(3 when interrupted at APPLICATION with standard UEFI TPL definitions).

**Pros:**
- Eliminates all 5 false-positive scenarios (10, 15, 18, 20, 25).
...
- Extends Solution 1's mask approach with bounded preemption — same IRET
  race elimination, callback-overwrite immunity, and ASSERT-validated
  monotonic nesting, but allows higher-priority event preemption.
- ASSERT validates monotonic nesting invariant.
...
- In ISR context, event callbacks at or below the highest interrupted TPL
  execute with interrupts disabled. Callbacks above the highest interrupted
  TPL execute with interrupts enabled (bounded preemption), allowing up to
  3 nested ISR levels with standard UEFI TPL definitions. This is more
  aggressive than Solution 1 (which disables interrupts for all ISR
  callbacks) but still bounded. Timer latency in normal context is
  unaffected.
...
| **Max nesting** | ∞ (stack overflow) | 0 (by design) | ≤ TPL count | 1 additional | ≤ pending TPLs | ≤ pending TPLs | ≤ TPL levels (3) |
...
| **False positives** | N/A | None | None | 5 scenarios | 5 scenarios | 5 scenarios | **None** |
...
3. **vs. Options C–E (Solutions 1–3):** All three use the *cached*
   `GetInterruptState()` value, which creates 5 false-positive scenarios
   where `CoreRestoreTpl` incorrectly suppresses interrupt re-enable in
   normal (non-ISR) context. Solution 4 eliminates these by reading the
...
5. **Bounded preemption:** The `mIsrEntryTplMask` approach extends
   Solution 1 to allow ISR nesting up to the number of dispatch TPL levels
   (max 3 with standard UEFI: APP → CALLBACK → NOTIFY).  Each nested ISR
   interrupts at a strictly higher TPL, and once all dispatch levels are
   consumed, no further nesting is possible. This provides immunity to
   callback overwrite, ASSERT-validated monotonic nesting, and no timer
   latency impact in normal operation.
...
This section defines all 25 scenarios and traces the behavior of the original
...
**Note:** Solution 4 (Option F) extends Solution 1's `mIsrEntryTplMask`
approach with two key differences: (1) the CPU Arch Protocol
`GetInterruptState()` fix reads actual hardware state instead of a cached
variable, and (2) the event dispatch loop uses bounded preemption
(`HighBitSet64(mask) < gEfiCurrentTpl`) instead of Solution 1's `== 0`
check, allowing ISR nesting up to the number of TPL levels (max 3).  For
TPL-managed scenarios (1–9) without ISR nesting, behavior is similar to
Solution 1 because hardware always clears the interrupt flag on ISR entry.
For direct manipulation scenarios (10–25), Solution 4 differs: what are
"false positives" for Solutions 1–3 (cached value) become "correct behavior"
for Solution 4 (reading true hardware state).  See Section 3.6.6 for the
full ISR nesting sequence diagram.
...
**DXE Core code extends Solution 1** with bounded preemption (same
`mIsrEntryTplMask` bitmask, but the event dispatch loop allows re-enable at
levels above the highest interrupted TPL rather than suppressing all
re-enable when any mask bit is set). The difference from Solutions 1–3 is in
the CPU Arch Protocol driver: `gCpu->GetInterruptState()` reads actual
hardware (RFLAGS.IF on x86, sstatus.SIE on RISC-V, DAIF on AArch64) instead
of returning a cached variable.
...
// DXE Core: mIsrEntryTplMask with bounded preemption
CoreRaiseTpl (NewTpl):
  ...
  if ((NewTpl >= TPL_HIGH_LEVEL) && (OldTpl < TPL_HIGH_LEVEL)) {
    if (gCpu != NULL) {
      gCpu->GetInterruptState (gCpu, &State);  // reads real HW
      if (!State) {
        ASSERT ((INTN)OldTpl >= HighBitSet64 (mIsrEntryTplMask));
        mIsrEntryTplMask |= (1ULL << OldTpl);
      }
...
CoreRestoreTpl (NewTpl):
  // event loop — bounded preemption (re-enable above highest interrupted):
    if ((HighBitSet64 (mIsrEntryTplMask) < (INTN)gEfiCurrentTpl) &&
        (gEfiCurrentTpl < TPL_HIGH_LEVEL)) {
      CoreSetInterruptState (TRUE);
    }
  ...
  // final section — unconditional disable, then early-return pattern:
  CoreSetInterruptState (FALSE);
  gEfiCurrentTpl = NewTpl;
  if (NewTpl >= TPL_HIGH_LEVEL) {
    return;                                    // stay at HIGH, interrupts off
  }
  if ((INTN)NewTpl <= HighBitSet64 (mIsrEntryTplMask)) {
    mIsrEntryTplMask &= (1ULL << NewTpl) - 1; // ISR unwind: clear bits
    return;                                    // leave disabled for IRET
  }
  CoreSetInterruptState (TRUE);                // normal context: re-enable
```

**Key behavioral differences from Solution 1:**

1. **Bounded preemption in event loop:** Instead of `mIsrEntryTplMask == 0`
   (which suppresses all interrupt re-enable when any mask bit is set), the
   condition `HighBitSet64(mIsrEntryTplMask) < gEfiCurrentTpl` allows
   interrupts to be re-enabled at dispatch levels above the highest
   interrupted TPL. This enables higher-priority events to preempt
   lower-priority ones even from interrupt context.

2. **`gCpu != NULL` guard:** Per PI Spec Vol 2, the platform boots with
   interrupts disabled until the CPU and Timer Architectural Protocols are
   installed. If gCpu is NULL, no ISR detection is needed.

3. **`CoreSetInterruptState(FALSE)` inside `gCpu != NULL`:** Only called
   when the CPU Arch Protocol is available (otherwise interrupts are already
   disabled per PI Spec boot requirements).

**Key behavioral difference from cached implementations:** Because
`GetInterruptState()` reads hardware:
...
positives in Scenarios 10, 15, 18, 20, 25.

### 4.2 TPL-Managed Scenarios (1–9)
...
- **Original:** Disables on raise, unconditionally re-enables on restore. ✅
- **Solution 1:** `GetInterruptState` → TRUE → mask unchanged → disables on
  raise, re-enables on restore. ✅
- **Solution 2:** `GetInterruptState` → TRUE → `mInterruptState = TRUE`
  → disables on raise, `mInterruptState = TRUE` → re-enables on restore. ✅
...
- **Solution 4:** Same as Solution 1 — HW reads IF=1 (interrupts
  enabled in normal context) → same result. ✅
...
- **All five:** No impact — inner guard prevents any change. ✅

**Scenario 4 — `RaiseTpl(HIGH)` inside interrupt handler:**
...
- **Solution 4:** Same bounded preemption as Solution 1 — HW reads IF=0 in ISR context
  → same mask behavior, same strictly bounded nesting (max 1 additional). ✅
...
- **Solution 2:** Inner `CoreTimerTick` sets flag FALSE. Inner `RestoreTpl`
  ...
  harmless (outer loop already has interrupts HW-enabled). ✅
...
- **Solution 4:** Same bounded preemption as Solution 1 — HW reads IF=0 on ISR entry
  → mask bit set → bounded nesting (max 1 additional). ✅

**Scenario 6 — Next `RaiseTpl(HIGH)` after stale flag/mask:**
After IRET, interrupts are HW-enabled but the flag/mask is stale.

- **Original:** N/A — no tracking mechanism.
- **Solution 1:** Next `RaiseTpl(HIGH)`: `GetInterruptState` → TRUE →
  mask not modified → self-cleans implicitly. ✅
- **Solution 2:** Next `RaiseTpl(HIGH)`: `GetInterruptState` → TRUE →
  `mInterruptState = TRUE` → self-cleans explicitly. ✅
- **Solution 3:** Mask is fully cleared during `RestoreTpl` (`&= (1<<NewTpl)-1`
  with NewTpl = interrupted TPL). No stale state after IRET. ✅
- **Solution 4:** Same as Solution 1 — mask is cleared during the ISR's own
  `RestoreTpl`, so no stale state exists even before the next raise. ✅

**Scenario 7 — Temporarily lower TPL across HIGH (normal context):**
...
- **Original:** `RestoreTpl` re-enables interrupts (correct in normal
  context). `RaiseTpl` re-disables. ✅
- **Solution 1:** Same — mask is 0, so enables are unconditional. ✅
- **Solution 2:** Same — flag is TRUE, so enables proceed. ✅
- **Solution 3:** Same — mask is 0, final section: `NewTpl > -1` → else
  branch → enables. ✅
- **Solution 4:** Same as Solution 1 — mask is 0 in normal context. ✅

If a timer interrupt fires during the lowered window, it follows Scenario 5
(all five handle it the same way — self-cleans on next `RaiseTpl`).

**Scenario 8 — Temporarily lower TPL across HIGH (interrupt context):**
...
- **Solution 4:** Same bounded preemption as Solution 1 — same mask-guarded
  event loop, same strictly bounded nesting (max 1 additional). ✅

**Scenario 9 — Normal TPL nesting through event dispatch loop:**
...
- **Original:** Each level unconditionally calls `CoreSetInterruptState(TRUE)`.
  Interrupts enabled during event dispatch at every level. ✅
- **Solution 1:** Mask is 0 (raised from normal context with interrupts on) →
  event loop checks `gEfiCurrentTpl > HighBitSet64(0) = -1` → always TRUE →
  enables interrupts at each dispatch level. Final: `CoreSetInterruptState(FALSE)`,
  `gEfiCurrentTpl = 4`, `4 > HighBitSet64(0) = -1` → else branch →
...
- **Solution 4:** Same as Solution 1 — HW reads IF=1, mask is 0,
  `HighBitSet64(mIsrEntryTplMask) = -1 < gEfiCurrentTpl` → enables at
  every dispatch level. ✅ Identical to original in normal context.
...
suppresses enables when `mIsrEntryTplMask` has bits at or above the current
dispatch level, which only occurs in interrupt context.
...
### 4.3 CPU Protocol Manipulation Scenarios (10–19)

These scenarios involve direct calls to `gCpu->EnableInterrupt()` /
`gCpu->DisableInterrupt()`, bypassing the TPL mechanism.

**Scenario 10 — Disable interrupts then `RaiseTpl(HIGH)` (normal context):**
Code manually disables interrupts, then calls `RaiseTpl(HIGH)`.

- **Original:** `CoreSetInterruptState(FALSE)` is redundant (already off).
  `RestoreTpl` unconditionally re-enables. ✅
- **Solution 1:** `GetInterruptState` → FALSE → sets mask bit →
  `RestoreTpl` skips enable → interrupts remain off. ⚠️ False positive
  (self-cleans when caller re-enables and next `RaiseTpl(HIGH)` occurs).
- **Solution 2:** `GetInterruptState` → FALSE → `mInterruptState = FALSE`
  → `RestoreTpl` skips enable. ⚠️ Same false positive, same self-clean.
- **Solution 3:** `GetInterruptState` → FALSE → sets mask bit (same as S1).
  Final: `CoreSetInterruptState(FALSE)`, `gEfiCurrentTpl = NewTpl`,
  `NewTpl ≤ HighBitSet64(mask)` → leaves disabled, clears bit. ⚠️ Same
  false positive. Self-cleans on `RestoreTpl` (mask cleared).
- **Solution 4:** `GetInterruptState` reads HW → IF=0 (truly disabled) →
...
**Scenario 11 — Enable interrupts in interrupt context:**
...
**Scenario 12 — Disable interrupts in interrupt context:**
Inside `CoreTimerTick`, code calls `gCpu->DisableInterrupt()`.

- **All five:** No-op — interrupts are already disabled. ✅

**Scenario 13 — Toggle interrupts, no TPL change:**
Code at a non-HIGH TPL toggles interrupts directly without any
`RaiseTpl(HIGH)` call.

- **All five:** No impact — the flag/mask is only written during
  `RaiseTpl` transitions to `TPL_HIGH_LEVEL`. ✅

**Scenario 14 — Enable interrupts then `RaiseTpl(HIGH)` in IRQ context:**
...
- **Solution 3:** `GetInterruptState` → TRUE → mask unchanged → treats as
  normal context. Same false negative. ❌
...
**Scenario 15 — App disables interrupts then `RaiseTpl(HIGH)`:**
Same as Scenario 10 but from application context.

- **Original:** Redundant disable, unconditional restore. ✅
- **Solution 1 / 2 / 3:** ⚠️ Same false positive as Scenario 10. Self-cleans.
- **Solution 4:** ✅ Correct — same as Scenario 10. Reads HW IF=0, respects
  caller's explicit disable.

**Scenario 16 — App disables interrupts, no TPL change:**
No `RaiseTpl(HIGH)` occurs — flag/mask never touched.

- **All five:** No impact. ✅

**Scenario 17 — App enables interrupts, no TPL change:**
Redundant enable at `TPL_APPLICATION` — no `RaiseTpl(HIGH)` occurs.

- **All five:** No impact. ✅

**Scenario 18 — App disables interrupts, later `RaiseTpl(HIGH)`:**
App disables early, does work, then eventually calls `RaiseTpl(HIGH)`.

- **Original:** Redundant disable, unconditional restore. ✅
- **Solution 1 / 2 / 3:** ⚠️ False positive for first `RaiseTpl/RestoreTpl`
  pair. Self-cleans when app re-enables and next `RaiseTpl(HIGH)` occurs.
- **Solution 4:** ✅ Correct — same as Scenario 10. Reads HW IF=0, respects
  caller's explicit disable.

**Scenario 19 — App enables interrupts at `TPL_HIGH`:**
Application raises to `TPL_HIGH_LEVEL`, then directly enables interrupts.
Severe misuse.

- **Original:** Timer fires → `CoreTimerTick` → `RestoreTpl` re-enables →
  timer fires again. ❌ Unpredictable.
- **Solution 1:** Inner `CoreTimerTick` sets mask → inner `RestoreTpl`
  skips enable. ⚠️ Mitigates (prevents inner re-enable).
- **Solution 2:** Inner `CoreTimerTick` sets flag → inner `RestoreTpl`
  skips enable. ⚠️ Same mitigation.
- **Solution 3:** Inner `CoreTimerTick` sets mask. Event loop enables
  (bounded nesting). Final: mask check → leaves disabled. ⚠️ Mitigates
...
- **Solution 4:** Same as Solution 1 — inner `CoreTimerTick` sets mask
  ...
  ⚠️ Mitigates (prevents inner re-enable).

### 4.4 Architecture-Specific Instruction Scenarios (20–25)

These scenarios use raw processor instructions (`CLI`/`STI`, `CPSID`/`CPSIE`,
`DAIFSet`/`DAIFClr`, etc.) to manipulate interrupt state. From the
perspective of `GetInterruptState()`, these produce identical results to the
CPU protocol calls in Scenarios 10–19.

| Architecture | Disable | Enable |
|---|---|---|
| **IA-32 / X64** | `CLI` | `STI` |
| **AArch64** | `MSR DAIFSet, #2` | `MSR DAIFClr, #2` |
| **ARM32** | `CPSID i` | `CPSIE i` |
| **RISC-V** | `CSRC sstatus, SIE` | `CSRS sstatus, SIE` |
| **LoongArch** | `CSRWR` clear `CRMD.IE` | `CSRWR` set `CRMD.IE` |

**Scenario 20 — CLI then `RaiseTpl(HIGH)` (normal context):**
Same as Scenario 10 — `GetInterruptState` returns FALSE due to software
disable.

- **Original:** Redundant disable, unconditional restore. ✅
- **Solution 1 / 2 / 3:** ⚠️ Same false positive. Self-cleans after STI +
  next `RaiseTpl(HIGH)`.
- **Solution 4:** ✅ Correct — same as Scenario 10. Reads HW IF=0 (CLI
  actually cleared the flag), respects the explicit disable.

**Scenario 21 — STI, no TPL change:**
Same as Scenario 17 — no `RaiseTpl(HIGH)` occurs.

- **All five:** No impact. ✅

**Scenario 22 — STI in interrupt context:**
Same as Scenario 11 — hardware immediately enables interrupts.

- **All five:** ❌ Cannot prevent. Recursion occurs before any software
  check.

**Scenario 23 — CLI in interrupt context:**
Same as Scenario 12 — redundant disable.

- **All five:** No-op. ✅

**Scenario 24 — STI at `TPL_HIGH`:**
Same as Scenario 19 — severe misuse.

- **Original:** ❌ Unpredictable.
- **Solution 1 / 2 / 3:** ⚠️ Mitigates inner re-enable.
- **Solution 4:** Same as Solution 1 — ⚠️ Mitigates (inner ISR sets mask).

**Scenario 25 — CLI/STI bracket (normal context):**
Code wraps a critical section with CLI/STI. No `RaiseTpl(HIGH)` inside.

- **All five:** No impact — flag/mask not involved. ✅

If `RaiseTpl(HIGH)` is called inside the bracket:

- **Original:** Redundant disable, unconditional restore, then outer STI.
  ✅ Correct net effect.
- **Solution 1 / 2 / 3:** ⚠️ False positive — `RestoreTpl` skips re-enable,
  but outer STI restores interrupts. Net effect correct; interrupts-off
  period slightly longer.
- **Solution 4:** ✅ Correct — `RestoreTpl` skips re-enable (HW reads IF=0
  inside CLI bracket), outer STI restores. This is correct: code explicitly
  disabled interrupts, so `RestoreTpl` should not re-enable them.
...
| 4 | `RaiseTpl(HIGH)` inside IRQ handler | Interrupt | TPL | ❌ **Recursion** | ✅ Bounded | ✅ Bounded | ✅ Bounded | ✅ Bounded |
| 5 | Timer IRQ during normal event dispatch | Normal→IRQ | TPL | ❌ **Recursion** | ✅ Bounded | ✅ Stale, harmless | ✅ Bounded | ✅ Bounded |
| 6 | Next `RaiseTpl(HIGH)` after stale | Normal | TPL | N/A | ✅ Self-cleans | ✅ Self-cleans | ✅ Mask cleared | ✅ Mask cleared |
| 7 | Temp lower TPL across HIGH (normal) | Normal | TPL | ✅ Correct | ✅ Correct | ✅ Correct | ✅ Correct | ✅ Correct |
| 8 | Temp lower TPL across HIGH (IRQ) | Interrupt | TPL | ❌ **Recursion** | ✅ Bounded | ✅ Bounded | ✅ Bounded | ✅ Bounded |
| 9 | Normal TPL nesting (event dispatch) | Normal | TPL | ✅ Correct | ✅ Correct | ✅ Correct | ✅ Correct | ✅ Correct |
| 10 | Disable IRQ then `RaiseTpl(HIGH)` | Normal | Protocol | ✅ Correct | ⚠️ False pos. | ⚠️ False pos. | ⚠️ False pos. | ✅ Correct |
| 11 | Enable IRQ in IRQ context | Interrupt | Protocol | ❌ Cannot fix | ❌ Cannot fix | ❌ Cannot fix | ❌ Cannot fix | ❌ Cannot fix |
| 12 | Disable IRQ in IRQ context | Interrupt | Protocol | ✅ No-op | ✅ No-op | ✅ No-op | ✅ No-op | ✅ No-op |
| 13 | Toggle IRQ, no TPL change | Normal | Protocol | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact |
| 14 | Enable IRQ + `RaiseTpl(HIGH)` in IRQ | Interrupt | Protocol | ❌ Cannot fix | ❌ False neg. | ❌ False neg. | ❌ False neg. | ❌ False neg. |
| 15 | App disables IRQ + `RaiseTpl(HIGH)` | App | Protocol | ✅ Correct | ⚠️ False pos. | ⚠️ False pos. | ⚠️ False pos. | ✅ Correct |
| 16 | App disables IRQ, no TPL | App | Protocol | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact |
| 17 | App enables IRQ, no TPL | App | Protocol | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact |
| 18 | App disables IRQ, later `RaiseTpl` | App | Protocol | ✅ Correct | ⚠️ False pos. | ⚠️ False pos. | ⚠️ False pos. | ✅ Correct |
| 19 | App enables IRQ at `TPL_HIGH` | App | Protocol | ❌ Unpredict. | ⚠️ Mitigates | ⚠️ Mitigates | ⚠️ Mitigates | ⚠️ Mitigates |
| 20 | CLI then `RaiseTpl(HIGH)` | Normal | Instruction | ✅ Correct | ⚠️ False pos. | ⚠️ False pos. | ⚠️ False pos. | ✅ Correct |
| 21 | STI, no TPL change | Normal | Instruction | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact | ✅ No impact |
| 22 | STI in IRQ context | Interrupt | Instruction | ❌ Cannot fix | ❌ Cannot fix | ❌ Cannot fix | ❌ Cannot fix | ❌ Cannot fix |
| 23 | CLI in IRQ context | Interrupt | Instruction | ✅ No-op | ✅ No-op | ✅ No-op | ✅ No-op | ✅ No-op |
| 24 | STI at `TPL_HIGH` | App | Instruction | ❌ Unpredict. | ⚠️ Mitigates | ⚠️ Mitigates | ⚠️ Mitigates | ⚠️ Mitigates |
| 25 | CLI/STI bracket (normal) | Normal | Instruction | ✅ Correct | ✅ / ⚠️ | ✅ / ⚠️ | ✅ / ⚠️ | ✅ Correct |

Legend: ✅ Correct | ⚠️ Minor regression or behavioral change | ❌ Broken / cannot fix

### 4.6 Conclusions

**For TPL-managed interrupt control (Scenarios 1–9):**
All four solutions correctly prevent infinite recursion. Solutions 2 and 3
allow bounded nesting during event dispatch (Scenarios 4/8: unconditionally
...
simplest). Normal TPL nesting through the event dispatch loop (Scenario 9)
...
**For direct interrupt manipulation (Scenarios 10–25):**
Solutions 1–3 behave identically — same false positives (10, 15, 18,
20, 25), same cannot-prevent cases (11, 14, 22), same no-impact cases
(12, 13, 16, 17, 21, 23, 25). **Solution 4 eliminates the false positives**
in Scenarios 10, 15, 18, 20, and 25 because `GetInterruptState()` reads
actual hardware state. When code explicitly disables interrupts (via
protocol or raw instruction), the hardware flag IS cleared, and Solution 4
correctly detects this and respects the caller's intent — `RestoreTpl` does
not re-enable, which is the correct behavior. The cannot-prevent cases (11,
14, 22) remain identical across all four solutions.

**Key distinction — "false positive" reframing for Solution 4:**
For Solutions 1–3 (cached `GetInterruptState`), Scenarios 10/15/18/20/25
are genuine false positives: the cache happens to read FALSE for reasons
unrelated to ISR entry. For Solution 4 (HW read), the same scenarios
produce **correct behavior**: if the hardware interrupt flag is cleared (by
explicit software action or by hardware ISR entry), `RestoreTpl` should NOT
re-enable interrupts — the caller or the interrupt-return instruction is
responsible for restoring them. This eliminates the "false positive"
category entirely for Solution 4.

**Original code tradeoff:** For the direct-disable scenarios (10, 15, 18,
20, 25), the original code's unconditional restore has a side effect:
it re-enables interrupts that the caller explicitly disabled. While this
prevents the interrupts-off-until-next-raise symptom, it violates the
caller's intent. Solution 4's behavior (respecting the explicit disable) is
more correct from a spec perspective — `DisableInterrupt()` means
"disable," and `RestoreTpl()` should not undo explicit interrupt
manipulation.
...
sequenceDiagram
    ...
    Note over DXE: Final: CoreSetInterruptState(FALSE)<br/>gEfiCurrentTpl = 4<br/>4 ≤ HighBitSet64(0x10)=4<br/>mask &= (1<<4)-1 = 0x0F → mask = 0<br/>Leave disabled ✅
    DXE-->>T1: returns (interrupts still disabled)

    T1-->>CPU: Return from interrupt (IRET)
    deactivate T1
    Note over CPU: HW re-enables interrupts on IRET ✅
...
- Same false-positive limitation as Solutions 2/3 for direct interrupt
  manipulation (Scenarios 10, 15, 18, 20, 25) when used standalone (without
  the Solution 4 CpuDxe fix). Solution 4 eliminates these false positives.
...
Interrupt handler active:
  ...
  CoreRestoreTpl(4):
    ...
    mInterruptState = FALSE → skip enable → interrupts stay off ✅
...
Next normal RaiseTpl:
  ...
  CoreRestoreTpl(4):
    ...
    Final (OldTpl=31 ≥ HIGH): CoreSetInterruptState(FALSE) → gEfiCurrentTpl=4
    mInterruptState = TRUE → enable interrupts ✅
```
...
sequenceDiagram
    ...
    Note over DXE: mInterruptState = FALSE<br/>Skip CoreSetInterruptState(TRUE) ✅
    DXE-->>T1: returns (interrupts disabled)

    T1-->>CPU: Return from interrupt
    deactivate T1
    Note over CPU: HW re-enables interrupts ✅
    ...
    Note over DXE: CoreSetInterruptState(FALSE)<br/>← disable before TPL assign
    Note over DXE: gEfiCurrentTpl = 4
    Note over DXE: mInterruptState = TRUE<br/>CoreSetInterruptState(TRUE) ✅
...
- Self-cleaning — no risk of stale bits accumulating in a mask.
...
| **False positives** | 5 scenarios (10, 15, 18, 20, 25) | 5 scenarios (10, 15, 18, 20, 25) | 5 scenarios (10, 15, 18, 20, 25) | **None** (reads HW) |
...
sequenceDiagram
    ...
    T1->>DXE: CoreRaiseTpl(31)
    Note over DXE: GetInterruptState → FALSE<br/>ASSERT(4 > HighBitSet64(0)=-1) ✅<br/>mInterruptedTplMask |= (1<<4) = 0x10
    DXE-->>T1: OldTpl = 4
    ...
    activate T2
    T2->>DXE: CoreRaiseTpl(31)
    Note over DXE: GetInterruptState → FALSE<br/>ASSERT(16 > HighBitSet64(0x10)=4) ✅<br/>mInterruptedTplMask |= (1<<16) = 0x10010
    DXE-->>T2: OldTpl = 16
...
- Same false-positive limitation as Solutions 1/2 for direct interrupt
  manipulation (Scenarios 10, 15, 18, 20, 25).
...
- **Solutions 1/4:** Dispatch at NOTIFY(16): `mInterruptedTplMask != 0` →
...
| **False positives** | 5 scenarios | 5 scenarios | 5 scenarios | **0 scenarios** |
...
5. **False positives:** Solution 4 eliminates all 5 false-positive scenarios
...
Final section:
  ...
  mInterruptState = FALSE → skip enable ✅
...
`gCpu->GetInterruptState()` → TRUE (event loop enabled them) →
`mInterruptState = TRUE`. This **overwrites** the outer ISR's FALSE value.
The outer `RestoreTpl`'s final section now sees `mInterruptState = TRUE` →
...
(`Bug_xxx_DxeCorePreventTimerInterruptInfiniteRecurssion_Solution_4`)

**Approach:** Two-part fix that combines Solution 1's DXE Core logic
(unchanged) with a spec-conformance fix to the CPU Architecture Protocol's
`GetInterruptState()` implementation. This eliminates the 5 false-positive
scenarios that affect Solutions 1–3 by making `GetInterruptState()` return
ground-truth hardware state instead of a stale cached variable.
...
**Part 2 — DXE Core logic (identical to Solution 1):**

Uses `mInterruptedTplMask` bitmask exactly as described in Section 5.1.
No code changes from Solution 1's DXE Core implementation.
...
| False positives (Scenarios 10,15,18,20,25) | 5 scenarios | **0 scenarios** |
...
In Scenarios 10, 15, 18, 20, and 25, code explicitly disables interrupts
(via protocol call or raw instruction) then calls `RaiseTpl(HIGH)`:

- **Solutions 1–3 (cached):** The cached variable was updated to FALSE by
  `DisableInterrupt()` or happens to be stale FALSE. `GetInterruptState()`
  returns FALSE → sets mask bit → `RestoreTpl` skips re-enable. This is a
  "false positive" because the code is NOT in interrupt context — it just
  has interrupts disabled by choice.

- **Solution 4 (HW read):** `GetInterruptState()` reads hardware IF=0.
  Sets mask bit → `RestoreTpl` skips re-enable. This is **correct behavior**:
  if code explicitly disabled interrupts, `RestoreTpl` should not
  unilaterally re-enable them. The caller is responsible for calling
  `EnableInterrupt()` when ready.

**Commits (5 total on branch):**

1. `MdeModulePkg/Core/Dxe: Prevent timer interrupt infinite recursion`
   — DXE Core `mInterruptedTplMask` logic (Part 2)
2. `UefiCpuPkg/CpuDxe: Read hardware interrupt state in GetInterruptState`
   — X64 CpuDxe fix (Part 1)
3. `UefiCpuPkg/CpuDxeRiscV64: Read hardware interrupt state in GetInterruptState`
   — RISC-V CpuDxe fix (Part 1)
4. `MdeModulePkg/Test: Add DxeCoreTplTest shell application`
   — Test suite
5. `OvmfPkg/LocalApicTimerDxe: Remove NestedInterruptTplLib dependency`
   — Removes now-unnecessary workaround from OVMF

**Pros (in addition to all Solution 1 pros):**
- Eliminates all 5 false-positive scenarios.
...
// Example: OVMF default = 100,000 (100ns units) = 10,000 µs = 10ms
//
...
InitTimerPeriod (VOID)
{
  ...
  if (mTimerPeriodUs == 0) {
    mTimerPeriodUs = 1;  // sub-microsecond timer — floor to 1µs
  }
}
...
#### 6.4.1 TPL-Managed Scenarios (1–9)
...
`RaiseTpl(HIGH)` / `RestoreTpl` cycle works correctly. The mask is already
cleared to 0 during the timer handler's own `CoreRestoreTpl` (via
`mIsrEntryTplMask &= (1ULL << NewTpl) - 1`), so there is no stale state
after IRET. This test confirms the mask self-cleans during the interrupt
handler, not after.
...
Directly verifies Scenario 5: a timer interrupt fires during normal-context
...
Steps:
  ...
  5. gBS->RestoreTPL(OldTpl)
     ...
     // → timer fires during Stall → CoreTimerTick runs (Scenario 5)
  ...
  12. ASSERT GetInterruptState() == TRUE  // self-cleaned ✅
  13. Close event
...
Verifies bounded preemption: a slow `TPL_CALLBACK` event handler is preempted
by a periodic `TPL_NOTIFY` timer event dispatched from a timer ISR's
`RestoreTpl`. The ISR's event loop condition
`HighBitSet64(mIsrEntryTplMask) < gEfiCurrentTpl` enables interrupts at
`TPL_NOTIFY` (above the interrupted `TPL_CALLBACK`), allowing the periodic
event to fire during the CALLBACK handler's stall. Maximum nesting depth is
bounded by the number of TPL levels (3 with standard UEFI).

```
Precondition: TPL_APPLICATION, interrupts enabled, timer running
Setup:
  Create periodic timer event at TPL_NOTIFY (fires every timer period)
    with callback that increments mNotifyDispatchCount
  Create EVT_NOTIFY_SIGNAL event at TPL_CALLBACK with slow callback:
    VOID EFIAPI PreemptSlowCallbackHandler (...) {
      mPreemptCallbackStarted = TRUE;
      mPreemptNotifyCountAtStart = mNotifyDispatchCount;
      gBS->Stall (StallForTicks(5));  // stall for several timer periods
      // With bounded preemption, timer ISR fires during stall,
      // dispatches the periodic NOTIFY event (preempts this CALLBACK)
...
Steps:
  ...
  2. OldTpl = gBS->RaiseTPL(TPL_HIGH_LEVEL)
  3. gBS->SignalEvent(SlowCallbackEvent)
  4. gBS->RestoreTPL(OldTpl)  // dispatches CALLBACK handler with IRQs enabled
  5. ClearRecursionWatchdog()
  6. ASSERT mPreemptCallbackStarted == TRUE
  7. ASSERT mPreemptCallbackFinished == TRUE
  8. ASSERT mPreemptNotifyCountAtEnd > mPreemptNotifyCountAtStart
     // Proves NOTIFY was dispatched DURING the CALLBACK handler
  9. // Verify system health
  10. OldTpl = gBS->RaiseTPL(TPL_HIGH_LEVEL)
  11. gBS->RestoreTPL(OldTpl)
  12. ASSERT GetInterruptState() == TRUE
  13. Close events
Expected: NOTIFY event preempts CALLBACK handler via bounded preemption,
no infinite recursion, system remains healthy
```

#### 6.4.2 CPU Protocol Manipulation Scenarios (10–19)
...
Verifies false-positive behavior: interrupts remain off after `RestoreTpl`,
and self-clean on next raise with interrupts enabled.
...
Steps:
  ...
  5. gBS->RestoreTPL(OldTpl)
  6. // False positive: interrupts remain OFF (expected for all solutions)
  7. ASSERT GetInterruptState() == FALSE  // ⚠️ known false positive
  8. gCpu->EnableInterrupt(gCpu)  // manually restore
  9. ASSERT GetInterruptState() == TRUE
  10. OldTpl = gBS->RaiseTPL(TPL_HIGH_LEVEL)  // self-clean
  11. gBS->RestoreTPL(OldTpl)
  12. ASSERT GetInterruptState() == TRUE  // self-cleaned ✅
Expected: Steps 6-7 show false positive; steps 10-12 show self-clean
```
...
#### 6.4.3 Architecture-Specific Instruction Scenarios (20–25)

These tests use architecture-specific inline assembly. Each test must be
conditionally compiled.

**Test 13 — CLI then `RaiseTpl(HIGH)` (X64/IA32 only)** *(Scenario 20)*
...
Steps:
  ...
  2. OldTpl = gBS->RaiseTPL(TPL_HIGH_LEVEL)
  3. gBS->RestoreTPL(OldTpl)
  4. ASSERT GetInterruptState() == FALSE  // ⚠️ false positive
  5. __asm__ volatile ("sti");   // or _enable() on MSVC
  6. OldTpl = gBS->RaiseTPL(TPL_HIGH_LEVEL)  // self-clean
  7. gBS->RestoreTPL(OldTpl)
  8. ASSERT GetInterruptState() == TRUE   // self-cleaned ✅
...
**Test 15 — STI at `TPL_HIGH` (X64/IA32 only)** *(Scenario 24)*
...
#### 6.4.4 IRQ-Context Tests via Timer Handler Hook (Scenarios 4, 5, 8)
...
→ FALSE), enabling direct verification of Scenarios 4, 5, and 8 that are
...
so `CoreRaiseTpl` sets the `mIsrEntryTplMask` bit for the interrupted
...
*(Scenarios 4, 8 — event dispatch path)*

Verifies that `CoreRestoreTpl`'s event dispatch loop re-enables interrupts
in IRQ context at dispatch levels above the interrupted TPL. The handler is
interrupted at `TPL_APPLICATION` (4), so the mask has bit 4 set. Dispatch at
`TPL_NOTIFY` (16) satisfies `16 > HighBitSet64(0x10) = 4`, so interrupts
are re-enabled. Events signaled before hooking are dispatched inside the
handler's `RestoreTpl` with interrupts re-enabled during dispatch.
...
above the interrupted TPL (mask check: 16 > 4 → enables)
```
...
no cumulative state corruption in `mIsrEntryTplMask`.
...
Directly exercises Scenario 8: inside an interrupt handler, temporarily
...
Without this raise, `mIsrEntryTplMask` is never set and the fix cannot
...
Handler:
  VOID EFIAPI Test19Handler (IN UINT64 Time) {
    ...
    //   mask check → 4 ≤ HighBitSet64(mask)=4 → leaves disabled ✅
...
Steps:
  ...
  11. // NOTE: Do not assert on mNotifyInterruptState / mCallbackInterruptState.
      // With the mask-guarded event loop, if a nested timer fires
...
**Test 23 — Non-HIGH RestoreTpl after timer interrupt** *(Scenario 5, 6 — regression)*

Verifies that a non-HIGH `RaiseTpl`/`RestoreTpl` cycle does not permanently
disable interrupts after a timer interrupt has occurred. For the
bitmask approach, this test trivially passes because: (a) the mask is already cleared
to 0 during the timer handler's own `CoreRestoreTpl` (via
`mIsrEntryTplMask &= (1ULL << NewTpl) - 1`), and (b) non-HIGH `RestoreTpl`
never consults `mIsrEntryTplMask` anyway — the mask check only occurs
...
Steps:
  ...
  2. Stall(StallForTicks(2))  // ≥ 2 ticks — ensure timer interrupt fires,
                               // leaving mIsrEntryTplMask potentially non-zero
  3. // Non-HIGH TPL cycle — must NOT check mIsrEntryTplMask
...
re-enables interrupts regardless of mIsrEntryTplMask value
```
...
interaction correctly. For the bitmask approach, the callback's
...
correctly and leaves interrupts disabled.
...
Handler:
  VOID EFIAPI Test24Handler (IN UINT64 Time) {
    ...
    // Final section: mask still has original IRQ bit → leaves disabled ✅
    gBS->RestoreTpl (OldTpl);
...
mIsrEntryTplMask (sees GetInterruptState→TRUE = normal context),
so outer final section correctly leaves interrupts disabled in IRQ context
```

**Test 25 — IRQ-context RestoreTpl to intermediate TPL** *(Scenario 4, 8 — partial restore)*
...
|---|---|---|---|---|
| 1 — Normal RaiseTpl/RestoreTpl | 1 | ✅ | ✅ | ✅ |
| 2 — RaiseTpl to non-HIGH | 2 | ✅ | ✅ | ✅ |
| 3 — Nested RaiseTpl(HIGH) | 3 | ✅ | ✅ | ✅ |
| 4 — No recursion under timer | 4, 5 | ✅ | ✅ | ✅ |
| 5 — Normal context after timer | 6 | ✅ | ✅ | ✅ |
| 6 — Temp lower across HIGH | 7 | ✅ | ✅ | ✅ |
| 7 — Normal TPL nesting | 9 | ✅ | ✅ | ✅ |
| 8 — Timer IRQ during dispatch | 5 | ✅ | ✅ | ✅ |
| 9 — DisableIRQ + RaiseTpl | 10, 15, 18 | ✅ | ✅ | ✅ |
| 10 — DisableIRQ at HIGH | 12 | ✅ | ✅ | ✅ |
| 11 — Toggle IRQ, no TPL | 13, 16, 17 | ✅ | ✅ | ✅ |
| 12 — EnableIRQ at HIGH | 19 | ✅ | ✅ | ✅ |
| 13 — CLI + RaiseTpl | 20 | ✅ (CLI) | ✅ (CLI) | ✅ (DAIFSet) |
| 14 — CLI/STI bracket | 21, 23, 25 | ✅ (CLI/STI) | ✅ (CLI/STI) | ✅ (DAIF) |
| 15 — STI at HIGH | 24 | ✅ (STI) | ✅ (STI) | ✅ (DAIFClr) |
| 16 — IRQ hook: RaiseTpl/RestoreTpl | 4, 8 | ✅ | ✅ | ✅ |
| 17 — IRQ hook: event dispatch | 4, 8 | ✅ | ✅ | ✅ |
| 18 — IRQ hook: sustained stress | 4, 5 | ✅ | ✅ | ✅ |
| 19 — IRQ hook: temp lower TPL | 8 | ✅ | ✅ | ✅ |
| 20 — Sustained stress | 4, 5 | ✅ | ✅ | ✅ |
| 21 — Rapid TPL cycling | 1–3, 9 | ✅ | ✅ | ✅ |
| 22 — Timer event verify | all | ✅ | ✅ | ✅ |
| 23 — Non-HIGH RestoreTpl regression | 5, 6 | ✅ | ✅ | ✅ |
...
**Untestable Scenarios:** Scenarios 11, 14, and 22 (enable interrupts in
interrupt context / enable interrupts then `RaiseTpl(HIGH)` in interrupt
context / STI in interrupt context) are marked ❌ Cannot fix in all three
implementations. These cannot be safely tested because they immediately
enable hardware interrupts inside an interrupt handler, which would cause
infinite recursion and system crash before any software guard can intervene.
These represent fundamental contract violations that no TPL-level fix can
address.

**IRQ-context coverage (Tests 16–19, 24, 25):** These tests provide direct
verification of Scenarios 4, 5, and 8 by hooking the timer interrupt to
execute test code in genuine interrupt context. Previously, these scenarios
were only testable indirectly (relying on the timer to fire naturally
during TPL cycling). The hook tests verify:
- `mIsrEntryTplMask` is correctly set in interrupt context (Test 16)
- Event dispatch loop enables interrupts in IRQ context (Test 17)
- No state corruption across multiple handler invocations (Test 18)
- Full event-dispatch + disable-before-assign path in IRQ context (Test 19)
- Callback `RaiseTpl(HIGH)` during IRQ-context event dispatch does not
  corrupt `mIsrEntryTplMask` (Test 24)
- Intermediate TPL restore (above interrupted level) correctly re-enables
  while final restore (at interrupted level) stays disabled (Test 25)

**Code-review-only verification:** The `CoreSetInterruptState(FALSE)` before
the final `gEfiCurrentTpl = NewTpl` assignment (the race window fix) cannot
be verified through runtime testing because the race window is a few
instructions wide and timing-dependent. This structural property is verified
by code review and the analysis in Section 5.4.1.
...
**FAIL (regression):** Tests 1–3, 5–8, or 23 fail interrupt state
assertions — indicates the fix is incorrectly suppressing interrupt
restoration in normal (non-interrupt) context. Tests 7 and 8 specifically
validate that the event dispatch loop's multi-level TPL nesting still
re-enables interrupts at each dispatch level. Test 23 specifically catches
the bug where non-HIGH `RestoreTpl` incorrectly consults the mask and
permanently disables interrupts.

**FAIL (IRQ-context):** Test 16 reports `mInterruptStateInHandler != FALSE`
— handler is not running in genuine interrupt context (test infrastructure
issue). Test 17 reports events not dispatched or interrupts not enabled
during dispatch — indicates the unconditional enable in the event loop is
not working correctly. Test 19 hangs — indicates bounded nesting is
failing during IRQ-context temporary TPL lowering.

**EXPECTED WARNINGS:** Tests 9 and 13 assert `GetInterruptState() == FALSE`
after `RestoreTpl` (the false-positive behavior). This is a known and
accepted consequence of direct interrupt manipulation bypassing the TPL
mechanism. If a future fix eliminates the false positive, these assertions
